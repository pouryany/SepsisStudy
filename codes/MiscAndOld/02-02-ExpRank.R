...
run1.exp.mat <- run1.exp.mat  %>%
                mutate_if(.,is.numeric,function(X){  XX  <- rank(X)
                                                    len <- length(X)
                                                    XX  <-  XX * (exp(-(XX/len)))
                                                    return(XX)}) 
...
pdf("images/ExpRank/unCorrectedPathwayIntesities.pdf")
...
plotMA(PathExpMat2[,3:5])
            ...
            pdf("images/ExpRank/PathwayDE570.pdf")
...
pdf("images/ExpRank/NormalizedPathwayIntesities.pdf")
...
pdf("images/ExpRank/PCAnormalizedGPL570.pdf")
...
pdf("images/ExpRank/ChildsHeatmapAllpaths.pdf")
...
pdf("images/ExpRank/ChildPathwayVariance.pdf")
...
pdf("images/ExpRank/ChildrenSubtypesGPL570.pdf")
...
write.csv(cutree(aa$tree_row, k = 4), "ExpRankchildClusters.CSV")
...
pdf("images/ExpRank/PCAnormalizedChilds570.pdf")
...
### Looking at adults. There not much subclasses here. We'll see what to do.
        ...
        pdf("images/ExpRank/adultPathwayVariance.pdf")
        ...
        pdf("images/ExpRank/adultrenSubtypesGPL570.pdf")
        ...
        write.csv(cutree(aa$tree_row, k = 4), "ExpRankadultClusters.CSV")
...
set.seed(1)
# Does not seem to be affected by study ... Yoohoo?
pdf("images/ExpRank/PCAuncorrected570.pdf")
autoplot(GPL570.pca, data = tab.order, colour = "study",shape = "stage")
dev.off()
...
## Working on GPL571
    ...
    run2.exp.mat <- run2.exp.mat  %>% mutate_if(.,is.numeric,function(X){  XX  <- rank(X)
                                                                            len <- length(X)
                                                                            XX  <-  XX * (exp(-(XX/len)))
                                                                            return(XX)}) 
#%>%mutate_if(.,is.numeric,function(X){X*X})
# Edit ranking scales above!
    ...
    pdf("images/ExpRank/unCorrectedPathwayIntesities571.pdf")
    ...
    pdf("images/ExpRank/CorrectedPathwayIntesities571.pdf")
    ...
    dev.off()
        ...
        #Check PCA for batch
            ...
            pdf("images/ExpRank/PCAnormalizedGPL571.pdf")
