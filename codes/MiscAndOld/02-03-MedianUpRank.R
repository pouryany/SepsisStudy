...
run1.exp.mat <- run1.exp.mat  %>%
                mutate_if(.,is.numeric,rank) 


zz <- as.matrix(run1.exp.mat[,2])
...
PathExpTab  <- PathExpTab %>% group_by(., Pathway) %>% 
               summarise_if(is.numeric,MedianUp)

pdf("images/MedianUpRank/unCorrectedPathwayIntesities.pdf")
...
plotMA(PathExpMat2[,3:5])
            ...
            head(tT,20)
            hist(tT$adj.P.Val)
            tT.select <- tT[ tT$adj.P.Val < 0.01 & abs(tT$logFC) > 2,]
            nrow(tT.select)
            
            labs <- as.data.frame(tab.order[,2:4])
            rownames(labs) <- tab.order$value
            pdf("images/MedianUpRank/PathwayDE570.pdf")
...
pdf("images/MedianUpRank/NormalizedPathwayIntesities.pdf")
...
pdf("images/MedianUpRank/PCAnormalizedGPL570.pdf")
...
pdf("images/MedianUpRank/ChildsHeatmapAllpaths.pdf")
...
pdf("images/MedianUpRank/ChildPathwayVariance.pdf")
...
pdf("images/MedianUpRank/ChildrenSubtypesGPL570.pdf")
...
write.csv(cutree(aa$tree_row, k = 4), "MedianUpRankchildClusters.CSV")
...
pdf("images/MedianUpRank/PCAnormalizedChilds570.pdf")
...
### Looking at adults. There not much subclasses here. We'll see what to do.
        ...
        pdf("images/MedianUpRank/adultPathwayVariance.pdf")
        ...
        pdf("images/MedianUpRank/adultrenSubtypesGPL570.pdf")
        ...
        write.csv(cutree(aa$tree_row, k = 4), "MedianUpadultClusters.CSV")
...
set.seed(1)
# Does not seem to be affected by study ... Yoohoo?
pdf("images/MedianUpRank/PCAuncorrected570.pdf")
autoplot(GPL570.pca, data = tab.order, colour = "study",shape = "stage")
dev.off()
...
# Edit ranking scales above!
    ...
    pdf("images/MedianUpRank/unCorrectedPathwayIntesities571.pdf")
    ...
    pdf("images/MedianUpRank/CorrectedPathwayIntesities571.pdf")
    ...
    dev.off()
        ...
        #Check PCA for batch
            ...
            pdf("images/MedianUpRank/PCAnormalizedGPL571.pdf")
...
## Putting both studies together          
            ...
            pheatmap(big.adult[vars > 0.2,],show_rownames = F,clustering_method = "ward.D2",
