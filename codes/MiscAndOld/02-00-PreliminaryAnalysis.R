...
GPL570.tab  <- tab.order   %>% group_by(.,study,stage) %>% summarise(., n = n())
...
#plotMA(PathExpMat2[,3:5])
            ...
            head(tT,20)
            hist(tT$adj.P.Val)
            tT.select <- tT[ tT$adj.P.Val < 0.05 & abs(tT$logFC) > 1,]
            nrow(tT.select)
            rownames(tT.select)
          
            write.csv(tT.select,"Reports/DEpathways.csv")
            
            labs <- as.data.frame(tab.order[,2:4])
            rownames(labs) <- tab.order$value
            pdf("images/PathwayDE570.pdf")
...
pdf("images/unCorrectedPathwayIntesities.pdf")
...
pdf("images/PCAnormalizedGPL570.pdf")
...
pdf("images/ChildsHeatmapAllpaths.pdf")
...
png("images/ChildPathwayVariance.png")
...
pdf("images/ChildrenSubtypesGPL570.pdf")
...
write.csv(cutree(aa$tree_row, k = 4), "childClusters.CSV")
...
pdf("images/PCAnormalizedChilds570.pdf")
...
### Looking at adults. There not much subclasses here. We'll see what to do.
        ...
        png("images/adultPathwayVariance.png")
        plotDensities(high.paths, main = "Pathway Variances in adults GPL570")
        dev.off()
        
        adult.mat  <- adult.mat[high.paths > .05, ]
        ...
        pdf("images/adultrenSubtypesGPL570.pdf")
        ...
        write.csv(cutree(aa$tree_row, k = 4), "adultClusters.CSV")
...
set.seed(1)
# Does not seem to be affected by study ... Yoohoo?
pdf("images/PCAuncorrected570.pdf")
autoplot(GPL570.pca, data = tab.order, colour = "study",shape = "stage")
dev.off()
...
# Edit ranking scales above!
    ...
    pdf("images/unCorrectedPathwayIntesities571.pdf")
    ...
    pdf("images/CorrectedPathwayIntesities571.pdf")
    ...
    dev.off()
        ...
        #Check PCA for batch
            ...
            pdf("images/PCAnormalizedGPL571.pdf")
...
## Putting both studies together          
            ...
            pdf("images/Alltogether.pdf")
            autoplot(big.pc, data = big.tab2, colour = "study", shape = "stage")
            dev.off()
            ...
            pdf("images/AlltogetherHeatMap.pdf")
            pheatmap(Big.mat[vars > 2,],show_rownames = F, annotation_col = big.tab2,
            ...
            pdf("images/AdultsAlltogetherHeatMap.pdf")
...
#Building a classifier for adults versus children
    ...
    set.seed(1)
    validation_index <- createDataPartition(big.tab2$stage, p=0.7, list=FALSE)
    gset.test      <-  Big.mat[,-validation_index]
    gene.exp.tot   <- Big.mat[,validation_index]        

    
    train.set     <- as_data_frame(t(gene.exp.tot)) 
    train.set$sml <- as.factor(big.tab2$stage)[validation_index]
    
    
    tests.set     <- as_data_frame(t(gset.test)) 
    tests.set.sml <- as.factor(big.tab2$stage)[-validation_index]
    ...
    lasso.feats <- lasso.feats[which(aa$importance > 0.03)] 
    ...
    set.seed(7)
    fit.glm <- train(sml ~ .,train.set[,lasso.feats], method = "glm", metric="ROC",
                     trControl=control)
    probs.glm <- predict(fit.glm,tests.set,type = "prob")
